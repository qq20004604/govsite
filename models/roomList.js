...
RoomList.prototype.userEnterRoom = function (roomID, userSocket, role, callback) {
    ...
    this.forEachUserSocketInRoom(room, function (member, role) {
        ...
        member.emit("alertToRoom", {
            ...
            msg: fun.getNowDate() + ": " + userSocket.userINFO.name + " 加入到房间" + roomID + "中，角色为：" + configInfo[role]
        })
    })
    //向房间内通报用户信息
...
RoomList.prototype.clearPlayer = function (userSocket, isUserLeave) {
    ...
    if (room) {
        ...
        this.forEachUserSocketInRoom(userSocket, function (member) {
            member.emit("alertToRoom", {
                msg: fun.getNowDate() + ": " + userSocket.userINFO.name + "离开了房间" + roomID + "，他的角色是：" + configInfo[role]
            })
        })
        //更新在线用户里，该用户所在的信息
...
RoomList.prototype.changeRole = function (userSocket, role) {
    ...
    if (role === oldRole) {
        userSocket.emit("alertToUser", {
            msg: fun.getNowDate() + ": " + "你已经是" + configInfo[role] + "了。"
        });
        return;
    }
    ...
    userSocket.emit("alertToUser", {
        msg: fun.getNowDate() + ": " + "角色切换成功，你现在是 " + configInfo[role] + " 了。"
    });
    //通报所有人，某人转换了角色
    this.forEachUserSocketInRoom(userSocket, function (member) {
        member.emit("alertToRoom", {
            msg: fun.getNowDate() + ": 房间" + oldRoomID + " " + userSocket.userINFO.name + " 将角色切换为 " + configInfo[role] + " 。"
        });
    })
}
